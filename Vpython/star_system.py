...
def dust_particles(n):
    ...
    for i in range(n):
        ...
        particles[i] = sphere(pos = rand_posvec()[0], radius = a, color = color.cyan,make_trail=True,retain=150, trail_radius=2e8)
...
def star_particles():
    star = sphere(pos = vector(0, 0, 0), radius = 400 + 200 * random.random(), color = vector(0.878, 0.208, 0.133),make_trail=True,retain=150, trail_radius=2e8)
    star.velocity = vector(0, 0, 0)
    star.trail_color=star.color
    return star
...
def zero_out(i):
    ...
    Type_Debs[i].trail_color= vector(0,0,0)
...
def particle_merge(i):
    ...
    for j in range(len(Type_Debs)):
        if (j == i) or (Type_Debs[j].visible == False):
            ...
            Type_Debs[i].trail_color=Type_Debs[i].color
            a.append(j)
        else:
            continue
